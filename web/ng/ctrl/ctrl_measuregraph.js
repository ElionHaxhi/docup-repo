function measureGraphCtrl($scope,  App, Patient, User, Graphui, Flexschema, Flexmodel, Exam) {
	 ...
	 *
	 */
	$scope.getFlextypeUsed = function() {
		//console.log("Measure graph init");
		//console.log($scope.exam.measureslist);
		var flextypesusedarr = {};
		_.each($scope.exam.measureslist, function(elem) {
		...
		return flextypesusedarr;
	};

	//$scope.graphparam.usedFlexforms = $scope.getFlextypeUsed();
			...
			_.each(flex.propsSelected, function(prop, propindex) {
				...
				_.each(Exam.list, function(exam) {
					if (exam.jsondata_type === flexindex) {
						var tick = moment(exam.examdate, 'YYYY-MM-DD').valueOf();
						var val = null;
						if (exam.jsondata) {
